  ...
  <div>
    <div class="search">
      ...
      <div class="common_hang">
        ...
        <el-select v-model="groupquerytype" placeholder="请选择" clearable>
          <el-option
            v-for="item in groupqueryList"
            :key="item.type"
            :label="item.name"
            :value="item.type"
          >
    ...
    <div class="rolesTree">
      <div class="common_hang">
        <div class="mech">选择套餐组</div>
        <el-select v-model="groupserialno" placeholder="请选择">
          <el-option
            v-for="item in rolesTree"
            :key="item.groupserialno"
            :label="item.groupname"
            :value="item.groupserialno"
          >
          </el-option>
        </el-select>
      </div>
      <el-button type="primary" @click="getCheckedKeys">关联套餐组</el-button>
    </div>
  </div>
</template>
...
<script>
import {
  querySaleplanGroupEffectiveList,
  querySimCardList,
  relationPlanGroup,
  getDict,
} from "../../api/api";
export default {
  ...
  created() {
    ...
    this.querySaleplanGroupEffectiveList();
    this.getDict();
  },
  methods: {
    getDict() {
    ...
    cardTypeChange() {
      (this.groupserialno = ""), this.querySaleplanGroupEffectiveList();
    },
    // 获取要关联的套餐组
    querySaleplanGroupEffectiveList() {
      var that = this;
      ...
      querySaleplanGroupEffectiveList(reporParams).then((res) => {
        if (res.code == "1") {
          that.$message({
            type: "error",
            duration: 3000,
    ...
    querySimCardList(page) {
      ...
      var reporParams = {
        userToken: sessionStorage.getItem("token"),
        groupquerytype: this.groupquerytype,
        cardtype: this.cardType,
        cardno: this.cardno,
    ...
    pageClick(page) {
      //点击分页
      this.pageNum = page;
      this.querySaleplanGroupEffectiveList(page);
    },
    ...
    //     if (!!item.groupname && typeof item.groupname == "object") {
    //       _this.getArray(item.groupname, name, tempArr);
    //     }
    //   });
    ...
    getCheckedKeys() {
      ...
      var reporParams = {
        userToken: sessionStorage.getItem("token"),
        groupserialno: this.groupserialno,
        cardserialnoArray: this.cardserialnoArray,
      };
      relationPlanGroup(reporParams).then((res) => {
        if (res.code == "0") {
          that.$message({
            type: "success",
            duration: 3000,
